...
def generate_place_fields_CI(n_fields, rad_range):
    ...
    for field in range(n_fields):
        ...
        while trie < maxtries and added == False:
            ...
            for cbar_ind, cbar in enumerate(field_c):
                #print('cbar', cbar)
                if np.linalg.norm(c-cbar) < radii[cbar_ind]:
                    # already a field there, try again
                    c = 2*np.random.rand(2) - 1
                else:
                    # add it to the list
                    #print('adding, no overlap', c)
                    field_c.append(c)
                    #print(len(field_c))
                    added = True
                    break
...
for ind, spikes1 in enumerate(spikes):
    ...
    tp2.build_binned_file_quick(tspikes, ttrials, tclust, windt, fs, ['Good'], period, '/home/brad/pcsim/placecellsimdat{}.binned'.format(ind), dt_overlap=dtovr)

# compute simplicial complexes
    print('Computing simplicial complexes...')
    with h5.File('/home/brad/pcsim/placecellsimdat{}.binned'.format(ind), 'r') as bf:
...
print('Saving...')
with open('/home/brad/environment_out_random_place_fields.pkl', 'wb') as f:
