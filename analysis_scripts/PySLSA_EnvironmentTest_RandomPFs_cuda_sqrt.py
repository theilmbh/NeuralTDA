...
for ind, spikes1 in enumerate(spikes):
    ...
    tp2.build_binned_file_quick(tspikes, ttrials, tclust, windt, fs, ['Good'], period, '/home/brad/pcsim/placecellsimdatcudasqrt{}.binned'.format(ind), dt_overlap=dtovr)

# compute simplicial complexes
    print('Computing simplicial complexes...')
    with h5.File('/home/brad/pcsim/placecellsimdatcudasqrt{}.binned'.format(ind), 'r') as bf:
...
for d1 in tqdm.tqdm(range(num_envs*ntrials)):
    for d2 in tqdm.tqdm(range(d1, num_envs*ntrials)):
        ...
        dists[d1, d2] = pyslsa.cuJS(SCGs_trials[d1], SCGs_trials[d2], dim, beta)

print('Saving...')
with open(os.path.join(figsavepth,'environment_out_random_place_fields_cuda_sqrt_{}_{}_{}.pkl'.format(ntrials, max_hole, nrepeats)), 'wb') as f:
